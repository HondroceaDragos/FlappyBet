...
def loadPlayerSprites(radius: float):
        ...
        idleSprite = pygame.image.load("../assets/sprites/playerSprites/idleSprite.png").convert_alpha()
        idleSprite = pygame.transform.smoothscale(idleSprite, (int(2 * radius), int(2 * radius)))

        flySprite = pygame.image.load("../assets/sprites/playerSprites/flySprite.png").convert_alpha()
        flySprite = pygame.transform.smoothscale(flySprite, (int(2 * radius), int(2 * radius)))

        dropSprite = pygame.image.load("../assets/sprites/playerSprites/dropSprite.png").convert_alpha()
        dropSprite = pygame.transform.smoothscale(dropSprite, (int(2 * radius), int(2 * radius)))
...
class Player:
    ...
    def decideState(self) -> None:
        ...
        if self.currPos.y + self._radius >= self.screen.get_height():
            ...
            elif self.jumpPressed and self.currPos.y + self._radius < self.screen.get_height():
                self.state = "FLYING"
            # Player is dropping
